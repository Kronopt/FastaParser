...
class Test__iter__:
    ...
    def test_single_fasta_file_rich(
        self,
        fasta_nucleotide_single,
        fasta_nucleotide_single_contents,
        fasta_aminoacid_single,
        fasta_aminoacid_single_contents,
    ):
        # nucleotide
        fasta_nucleotide_reader = Reader(
            fasta_nucleotide_single, sequences_type="nucleotide"
        )
        fastas_nucleotide = []
        ...
        assert (
            fastas_aminoacid[0].sequence_as_string()
    ...
    def test_single_fasta_file_quick(
        self,
        fasta_nucleotide_single,
        fasta_nucleotide_single_contents,
        fasta_aminoacid_single,
        ...
        assert fastas_nucleotide[0].header == ">" + " ".join(
            (
                fasta_nucleotide_single_contents[0][0],
        ...
        assert fastas_aminoacid[0].header == ">" + " ".join(
            (
                fasta_aminoacid_single_contents[0][0],
                fasta_aminoacid_single_contents[0][1],
            )
        )

    def test_multiple_fasta_file_rich(
        self,
        ...
        # nucleotide
        fasta_nucleotide_reader = Reader(
            fasta_nucleotide_multiple, sequences_type="nucleotide"
        )
        fastas_nucleotide = []
        for fasta in fasta_nucleotide_reader:
            fastas_nucleotide.append(fasta)
            assert (
                fasta.sequence_as_string()
                == fasta_nucleotide_multiple_contents[len(fastas_nucleotide) - 1][2]
            )
            assert (
                fasta.id
                == fasta_nucleotide_multiple_contents[len(fastas_nucleotide) - 1][0]
            )
            assert (
                fasta.description
        ...
        # aminoacid
        fasta_aminoacid_reader = Reader(
            fasta_aminoacid_multiple, sequences_type="aminoacid"
        )
        fastas_aminoacid = []
        for fasta in fasta_aminoacid_reader:
            fastas_aminoacid.append(fasta)
            assert (
                fasta.sequence_as_string()
                == fasta_aminoacid_multiple_contents[len(fastas_aminoacid) - 1][2]
            )
            assert (
                fasta.id
                == fasta_aminoacid_multiple_contents[len(fastas_aminoacid) - 1][0]
            )
            assert (
                fasta.description
    ...
    def test_multiple_fasta_file_quick(
        self,
        fasta_nucleotide_multiple,
        fasta_nucleotide_multiple_contents,
        fasta_aminoacid_multiple,
        ...
        # nucleotide
        fasta_nucleotide_reader = Reader(
            fasta_nucleotide_multiple, parse_method="quick"
        )
        fastas_nucleotide = []
        for fasta in fasta_nucleotide_reader:
            fastas_nucleotide.append(fasta)
    ...
    def test_empty_lines_between_fastas(
        self, fasta_multiple_empty_lines, fasta_multiple_empty_lines_contents
    ):
        ...
        for fasta in fasta_reader:
            ...
            assert (
                fasta.sequence
                == fasta_multiple_empty_lines_contents[len(fastas) - 1][2]
            )
            assert fasta.header == ">" + " ".join(
                (
                    fasta_multiple_empty_lines_contents[len(fastas) - 1][0],
                    fasta_multiple_empty_lines_contents[len(fastas) - 1][1],
                )
            )
        assert len(fastas) == 2

    def test_fasta_no_empty_lines(
        self, fasta_no_empty_lines, fasta_no_empty_lines_contents
    ):
    ...
    def test_fasta_empty_lines_in_sequence(
        self, fasta_empty_lines_in_sequence, fasta_empty_lines_in_sequence_contents
    ):
        ...
        for fasta in fasta_reader:
            ...
            assert (
                fasta.sequence
                == fasta_empty_lines_in_sequence_contents[len(fastas) - 1][2]
            )
            assert fasta.header == ">" + " ".join(
                (
                    fasta_empty_lines_in_sequence_contents[len(fastas) - 1][0],
...
class Test__next__:
    def test_existing_current_iterator(
        self, fasta_nucleotide_multiple, fasta_nucleotide_multiple_contents
    ):
        fasta_reader = Reader(fasta_nucleotide_multiple, sequences_type="nucleotide")
        iter(fasta_reader)
...
class Test__repr__:
    def test__repr__(self, fasta_empty):
        fasta_reader = Reader(fasta_empty)
        assert repr(fasta_reader) == "fastaparser.Reader(%s)" % os.path.abspath(
            fasta_empty.name
        )
